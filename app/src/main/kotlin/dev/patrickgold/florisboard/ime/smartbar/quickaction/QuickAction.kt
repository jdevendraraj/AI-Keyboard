...
fun QuickAction.computeDisplayName(evaluator: ComputingEvaluator): String {
    return when (this) {
        is QuickAction.InsertKey -> stringRes(when (data.code) {
            ...
            KeyCode.CYCLE_PROMPT_MODE -> R.string.quick_action__cycle_prompt_mode
...
fun QuickAction.computeTooltip(evaluator: ComputingEvaluator): String {
    return when (this) {
        is QuickAction.InsertKey -> when (data.code) {
            KeyCode.CYCLE_PROMPT_MODE -> {
                val context = evaluator.context()
                if (context != null) {
                    val selectedMode = getSelectedMode(context)
                    val modeTitle = selectedMode?.title ?: "Default"
                    context.getString(R.string.quick_action__cycle_prompt_mode__tooltip, modeTitle)
                } else {
                    "Prompt Mode"
                }
            }
            else -> stringRes(when (data.code) {
